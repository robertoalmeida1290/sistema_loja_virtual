...
public class AcessoController {
	...
	public ResponseEntity<Acesso> salvarAcesso(@RequestBody Acesso acesso) throws ExceptionSistemaJava { /*Recebe o JSON e converte pra Objeto*/
		
		if (acesso.getId() == null) {
		  List<Acesso> acessos = acessoRepository.buscarAcessoDesc(acesso.getDescricao().toUpperCase());
		  
		  if (!acessos.isEmpty()) {
			  throw new ExceptionSistemaJava("Já existe Acesso com a descrição: " + acesso.getDescricao());
		  }
		}
	...
	public ResponseEntity<Acesso> obterAcesso(@PathVariable("id") Long id) throws ExceptionSistemaJava { 
		
		Acesso acesso = acessoRepository.findById(id).orElse(null);
		
		if (acesso == null) {
			throw new ExceptionSistemaJava("Não encontrou Acesso com código: " + id);
		}
