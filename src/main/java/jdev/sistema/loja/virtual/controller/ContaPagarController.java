...
import jdev.sistema.loja.virtual.repository.ContaPagarRepository;
...
public class ContaPagarController {
	...
	public ResponseEntity<ContaPagar> salvarAcesso(@RequestBody @Valid ContaPagar contaPagar) throws ExceptionSistemaJava { /*Recebe o JSON e converte pra Objeto*/
		...
		if (contaPagar.getPessoa() == null || contaPagar.getPessoa().getId() <= 0) {
			throw new ExceptionSistemaJava("Pessoa responsável deve ser informada");
		}
		
		if (contaPagar.getPessoa_fornecedor() == null || contaPagar.getPessoa_fornecedor().getId() <= 0) {
			throw new ExceptionSistemaJava("Fornecedor responsável deve ser informada");
		}
		...
		if (contaPagar.getId() == null) {
			...
			if(!contaPagars.isEmpty()) {
				throw new ExceptionSistemaJava("Já existe conta a pagar com a mesma descrição.");
			}
		}
	...
	public ResponseEntity<ContaPagar> obterContaPagar(@PathVariable("id") Long id) throws ExceptionSistemaJava { 
		...
		if (contaPagar == null) {
			throw new ExceptionSistemaJava("Não encontrou Conta a PAgar com código: " + id);
		}
